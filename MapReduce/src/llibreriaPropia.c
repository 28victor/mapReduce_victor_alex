...
// Funcio que retorna un bool (0|1) en cas que el fitxer que l'usuari ha escollit existeixi o no.
bool escullFitx(char ruta[])
{
    // Definim bool resultat.
    bool correcte = false;
    char nomAux[MAXCADENA];

    // Demanem a l'usuari que introdueixi el nom del fitxer sense l'extensiÛ.
    printf("\nEscull el fitxer per analitzar: [Sense .txt]\n");
    entrarCadena(nomAux, MAXCADENA);

    // Concatenem el que l'usuari ha escollit amb la ruta base (./fitxers/) i l'extensiÛ <.txt>.
    strcat(ruta, nomAux);
    strcat(ruta, ".txt");

    // Natajem la pantalla i executem una animaciÛ de proces.
    cls();
    anim();

    // Executem la funciÛ "existeixFitxer" on es comproba que el fitxer existeix.
    if (existeixFitxer(ruta) == true)
    {
        // Comprobem que el fitxer escollit no estigui buit.
        if (fitxerBuit(ruta) == false)
        {
            // Avisem al usuari que el fitxer ha sigut seleccionat correctament.
            printf("\nEl fitxer s'ha seleccionat correctament.");
            // Cambiem el valor de correcte (bool) a "true" .
            correcte = true;
            // Amb la funciÛ "continua()" l'hi demanem al usuari que pulsi per continuar.
            continua();
            cls();
        }
    }
